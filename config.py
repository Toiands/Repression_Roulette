...
每轮你会遇到一位陌生女嘉宾。可先试探（试纸、问病史、看报告等），再选择亲密程度——**试纸/病史反映的是感染风险，防护态度与可选行动要看「坦诚沟通防护」等对话**，二者未必一致（存在「雷」）。感染性病后**当轮就会受到较大健康冲击**，潜伏期结束后还会持续扣血。越到后期越难遇到「看起来安全」的人；一味全程保护能保命，但压抑会缓慢爬升，孤独也会叠加。侧边栏可查看**成就**（同一浏览器会尽量保留解锁记录）。谁更安全只能靠自己判断，后果常在事后才显现。通关或失败时可在**本局嘉宾回顾**中查看遇到过的人。
